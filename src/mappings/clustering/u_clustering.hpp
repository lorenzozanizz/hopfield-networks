#pragma once
#ifndef U_CLUSTERING_HPP 
#define U_CLUSTERING_HPP 
...
private:
	...
	double computing_distance(int idx_1, int idx_2) {
		return Utilities::euclidean_distance(
			trained_map.get_weights(idx_1),
			trained_map.get_weights(idx_2),
			trained_map.get_input_size()
		);
...
public:
	UClustering(KonohenMap<DataType>& trained_map, double threshold) : trained_map(trained_map), threshold(threshold) {
		heigth = trained_map.get_map_height();
		width = trained_map.get_map_width();
		u_heigth = (2 * heigth - 1);
		u_width = (2 * width - 1);
		u = std::vector<double>(u_heigth* u_width, 0.0);
	}
	...
	double neighb_mean(int x,int y) {
		double sum = 0.0;
		int count = 0;
		//std::cout << "computing neighbour mean of " << "(" << x << ", " << y << ")\n";
		...
		//if ((sum / count) < 0) std::cout << "sum is " << sum << "\n";
		return sum / count;
	}
