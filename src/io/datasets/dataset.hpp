...
public:
    ...
    VectorCollection(size_t input_size) : input_size(input_size) {
        // Resize all the data vectors
        x_data.reserve(input_size); id_data.reserve(input_size);
        // Initialize the permutation
        permutation.resize(input_size);
        std::iota(permutation.begin(), permutation.end(), 0);
    }
    ...
    void add_sample(const XType& x, const DataID id) {
        x_data.push_back(std::move(x));
        id_data.push_back(id);
    }
    ...
    class BatchView {
        ...
        const VectorCollection& parent;
        size_t start;
        size_t count;

    public:

        BatchView(const VectorCollection& p, size_t s, size_t c) :
            parent(p), start(s), count(c) {
        }
    ...
    public:
        ...
        BatchIterator(const VectorCollection* p, size_t bs, size_t idx)
            : parent(p), batch_size(bs), batch_index(idx) {
        }
